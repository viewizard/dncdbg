...
static bool stream_is_ident_start(Stream *self) {
  return iswalnum((wint_t)self->next) || self->next == '_' || self->next == '-';
}
...
static uint16_t symbol_table_insert_name(
  ...
  *array_get(&self->characters, self->characters.size - 1) = 0;
  ...
  return (uint16_t)(self->slices.size - 1);
}
    ...
    for (unsigned j = 0; j < analysis->states.size; j++) {
      ...
      while (ts_lookahead_iterator__next(&lookahead_iterator)) {
        ...
        while (node_index < subgraph->nodes.size) {
          ...
          // to this child node.
          TSSymbol alias = (TSSymbol)ts_language_alias_at(self->language, node->production_id, child_index);
...
static bool ts_query__analyze_patterns(TSQuery *self, unsigned *error_offset) {
  ...
  for (unsigned i = 0; i < self->pattern_map.size; i++) {
    ...
    if (!pattern->is_rooted) {
      ...
      if (step->symbol != WILDCARD_SYMBOL) {
        array_push(&non_rooted_pattern_start_steps, (uint16_t)i);
      }
    }
  }
  ...
  for (unsigned i = 0; i < parent_step_indices.size; i++) {
    uint16_t parent_step_index = (uint16_t)*array_get(&parent_step_indices, i);
  ...
  for (unsigned i = 0; i < self->patterns.size; i++) {
    ...
    for (
      ...
      if (step->type == TSQueryPredicateStepTypeCapture) {
        uint16_t value_id = (uint16_t)step->value_id;
...
static void ts_query__add_negated_fields(
  ...
  for (unsigned i = 0; i < self->negated_fields.size; i++) {
    ...
    if (existing_field_id == 0) {
      if (match_count == field_count) {
        step->negated_field_list_id = (uint16_t)start_i;
        return;
      } else {
        start_i = i + 1;
        match_count = 0;
  ...
  step->negated_field_list_id = (uint16_t)self->negated_fields.size;
...
static TSQueryError ts_query__parse_pattern(
  ...
  if (
    ...
    array_push(&self->step_offsets, ((StepOffset) {
      .step_index = (uint16_t)starting_step_index,
  ...
  if (stream->next == '[') {
    ...
    for (;;) {
      ...
      array_push(&branch_step_indices, (uint16_t)start_index);
      array_push(&self->steps, query_step__new(0, (uint16_t)depth, false));
    ...
    for (unsigned i = 0; i < branch_step_indices.size - 1; i++) {
      uint16_t step_index = (uint16_t)*array_get(&branch_step_indices, i);
      uint16_t next_step_index = (uint16_t)*array_get(&branch_step_indices, i + 1);
      ...
      end_step->alternative_index = (uint16_t)self->steps.size;
  ...
  // * A named node
    ...
    // Otherwise, this parenthesis is the start of a named node.
      ...
      array_push(&self->steps, query_step__new(symbol, (uint16_t)depth, is_immediate));
      ...
      for (;;) {
        ...
        uint16_t step_index = (uint16_t)self->steps.size;
        TSQueryError e = ts_query__parse_pattern(
          self,
          stream,
          depth + 1,
  ...
  // Parse a wildcard pattern
    ...
    array_push(&self->steps, query_step__new(WILDCARD_SYMBOL, (uint16_t)depth, is_immediate));
  }
  ...
  switch (quantifier) {
    case TSQuantifierOneOrMore:
      repeat_step = query_step__new(WILDCARD_SYMBOL, (uint16_t)depth, false);
      repeat_step.is_inside_alternation = is_inside_alternation;
      repeat_step.alternative_index = (uint16_t)starting_step_index;
    ...
    case TSQuantifierZeroOrMore:
      ...
      step->alternative_index = (uint16_t)self->steps.size;
      break;
    case TSQuantifierZeroOrOne:
      step = array_get(&self->steps, starting_step_index);
    ...
    *error_type = ts_query__parse_pattern(self, &stream, 0, false, false, &capture_quantifiers);
    ...
    for (;;) {
      ...
      ts_query__pattern_map_insert(self, step->symbol, (PatternEntry) {
        .step_index = (uint16_t)start_step_index,
    ...
    // next branch. After a quantified branch matches, it loops back to the cleaned copy.
    {
      ...
      for (uint32_t i = pat_start; i < pat_end; i++) {
        ...
        redirect.alternative_index = (uint16_t)(target_idx + 1);
        ...
        s->alternative_index = (uint16_t)copy_idx;
      }
    }
  }
...
const char *ts_query_capture_name_for_id(
  ...
  return symbol_table_name_for_id(&self->captures, (uint16_t)index, length);
}
...
TSQuantifier ts_query_capture_quantifier_for_id(
  ...
  return capture_quantifier_for_id(capture_quantifiers, (uint16_t)capture_index);
}
...
const char *ts_query_string_value_for_id(
  ...
  return symbol_table_name_for_id(&self->predicate_values, (uint16_t)index, length);
}
...
void ts_query_disable_capture(
  ...
  if (id != -1) {
    for (unsigned i = 0; i < self->steps.size; i++) {
      QueryStep *step = array_get(&self->steps, i);
      query_step__remove_capture(step, (uint16_t)id);
    }
  }
}
...
static void ts_query_cursor__add_state(
  ...
  array_insert(&self->states, (uint16_t)index, ((QueryState) {
    .id = UINT32_MAX,
    .capture_list_id = CAPTURE_LIST_NONE,
    .heap_insert_order = UINT32_MAX,
    .step_index = (uint16_t)pattern->step_index,
    .pattern_index = (uint16_t)pattern->pattern_index,
...
static inline bool ts_query_cursor__should_descend(
  ...
  for (unsigned i = 0; i < self->states.size; i++) {
    ...
    if (
      next_step->depth != PATTERN_DONE_MARKER &&
      (uint32_t)(state->start_depth + next_step->depth) > self->depth
    ) {
      return true;
    }
  }
  ...
  for (;;) {
    ...
    // Enter a new node.
      ...
      if (node_within_containing_range && self->on_visible_node) {
        TSSymbol symbol = (TSSymbol)ts_node_symbol(node);
...
bool ts_query_cursor_next_match(
  ...
  match->capture_count = (uint16_t)captures->size;
  capture_list_pool_release(&self->capture_list_pool, state->capture_list_id);
  if (self->finished_states_heap_size > 0) {
...
bool ts_query_cursor_next_capture(
  ...
  for (;;) {
    ...
    if (state) {
      ...
      match->captures = captures->contents;
      match->capture_count = (uint16_t)captures->size;
      *capture_index = (uint16_t)state->consumed_capture_count;
      state->consumed_capture_count++;
