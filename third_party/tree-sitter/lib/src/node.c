...
static inline bool ts_node__is_relevant(TSNode self, bool include_anonymous) {
  ...
  if (include_anonymous) {
    ...
    TSSymbol alias = (TSSymbol)ts_node__alias(&self);
    if (alias) {
...
static inline TSNode ts_node__first_child_for_byte(
  ...
  NodeChildIterator last_iterator = {0};
...
TSSymbol ts_node_symbol(TSNode self) {
  TSSymbol symbol = (TSSymbol)ts_node__alias(&self);
...
char *ts_node_string(TSNode self) {
  TSSymbol alias_symbol = (TSSymbol)ts_node__alias(&self);
...
bool ts_node_is_error(TSNode self) {
  TSSymbol symbol = (TSSymbol)ts_node_symbol(self);
