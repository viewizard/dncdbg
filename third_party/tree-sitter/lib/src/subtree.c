...
void ts_subtree_set_symbol(
  ...
  if (self->data.is_inline) {
    ...
    self->data.symbol = (uint8_t)symbol;
  ...
  if (self.ptr->child_count > 0) {
    ...
    if (
      ...
      if (ts_subtree_repeat_depth(first_child) > ts_subtree_repeat_depth(last_child)) {
        self.ptr->repeat_depth = (uint16_t)(ts_subtree_repeat_depth(first_child) + 1);
      } else {
        self.ptr->repeat_depth = (uint16_t)(ts_subtree_repeat_depth(last_child) + 1);
      }
    }
  }
}
...
MutableSubtree ts_subtree_new_node(
  ...
  *data = (SubtreeHeapData) {
    .ref_count = 1,
    .symbol = symbol,
    .child_count = (uint16_t)children->size,
    .visible = metadata.visible,
    .named = metadata.named,
    .has_changes = false,
    .has_external_scanner_state_change = false,
    ...
    {{
      ...
      .production_id = (uint16_t)production_id,
      .first_leaf = {.symbol = 0, .parse_state = 0},
    }}
  };
  ...
  while (stack.size) {
    ...
    if (result.data.is_inline) {
      if (ts_subtree_can_inline(padding, size, lookahead_bytes)) {
        ...
        data->error_cost = 0;
        data->child_count = 0;
        data->symbol = (TSSymbol)result.data.symbol;
        ...
        data->is_missing = (uint8_t)result.data.is_missing;
        data->is_keyword = (uint8_t)result.data.is_keyword;
        result.ptr = data;
      }
    } else {
