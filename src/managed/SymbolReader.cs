...
public class SymbolReader
{
    ...
    private class TargetStream : Stream
    {
        ...
        public TargetStream(ulong address, int size, ReadMemoryDelegate readMemory) : base()
        {
            _address = address;
    ...
    /// </param>
    /// <param name="isFileLayout">type of in-memory PE layout, if true, file based layout otherwise, loaded
    /// layout</param> <param name="loadedPeAddress"> Loaded PE image address or zero if the module is dynamic
    /// (generated by Reflection.Emit). Dynamic modules have their PDBs (if any) generated to an in-memory stream
    ...
    /// <returns>"Ok" if information is available</returns>
    private static RetCode GetNextUserCodeILOffset(IntPtr symbolReaderHandle, int methodToken, uint ilOffset,
                                                   out uint ilNextOffset, out int noUserCodeFound)
    {
        Debug.Assert(symbolReaderHandle != IntPtr.Zero);
        ilNextOffset = 0;
        noUserCodeFound = 0;
                                                   ...
                                                   uint normalNum, IntPtr normalTokens, out IntPtr data)
    {
        ...
        try
        {
            ...
            Dictionary<DocumentHandle, List<method_data_t>> ModuleData =
                new Dictionary<DocumentHandle, List<method_data_t>>();
                                               ...
                                               [MarshalAs(UnmanagedType.LPWStr)] string sourcePath, out IntPtr data)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < tokenNum; i++)
            {
                ...
                // Note, we don't check that current_p was found or not, since we know for sure, that sourceLine could
                // be resolved in method. Same idea for nested_p below, if we have nestedToken - it will be resolved for sure.
                ...
                if (nestedToken != 0)
                {
                    ...
                    if ((nested_start_p.StartLine > current_p.StartLine ||
                         (nested_start_p.StartLine == current_p.StartLine &&
                          nested_start_p.StartColumn > current_p.StartColumn)) &&
                        (nested_end_p.EndLine < current_p.EndLine ||
                         (nested_end_p.EndLine == current_p.EndLine && nested_end_p.EndColumn < current_p.EndColumn)))
                    {
                        list.Add(
                            new resolved_bp_t(current_p.StartLine, current_p.EndLine, current_p.Offset, methodToken));
                        break;
                    }

                    // Note, sequence points can't partially overlap each other, since same lemmas can't belong to 2
                    // different sequence points for sure. In this case we could check not "line" (start line - end line
                    // datas) but only "point" (end line data) for current method sequence point and first nested method
                    // sequence point.
                    if (current_p.EndLine > nested_start_p.EndLine ||
                        (current_p.EndLine == nested_start_p.EndLine && current_p.EndColumn > nested_start_p.EndColumn))
                    {
                        list.Add(new resolved_bp_t(nested_start_p.StartLine, nested_start_p.EndLine, nested_start_p.Offset, nestedToken));
                        // (tokenNum > 1) can have only lines, that added to multiple constructors, in this case - we
                        // will have same for all Tokens, we need unique tokens only for breakpoints, prevent adding
                        // nestedToken multiple times.
                        break;
                    }
                }
                                                         ...
                                                         out IntPtr localVarName, out int ilStartOffset, out int ilEndOffset)
    {
        ...
        try
        {
            ...
            if (!GetLocalVariableAndScopeByIndex(symbolReaderHandle, methodToken, localIndex, out localVar,
                                                 out ilStartOffset, out ilEndOffset))
                return RetCode.Fail;
    ...
    /// <returns>"Ok" if information is available</returns>
    internal static RetCode GetHoistedLocalScopes(IntPtr symbolReaderHandle, int methodToken, out IntPtr data,
                                                  out int hoistedLocalScopesCount)
    {
        data = IntPtr.Zero;
    ...
    private static void ReadPortableDebugTableEntries(PEReader peReader, out DebugDirectoryEntry codeViewEntry,
                                                      out DebugDirectoryEntry embeddedPdbEntry)
    {
    ...
    private static OpenedReader TryOpenReaderFromCodeView(PEReader peReader, DebugDirectoryEntry codeViewEntry,
                                                          string assemblyPath)
    {
        OpenedReader result = null;
        MetadataReaderProvider provider = null;
    ...
    internal static RetCode GetAsyncMethodSteppingInfo(IntPtr symbolReaderHandle, int methodToken, out IntPtr asyncInfo,
                                                       out int asyncInfoCount, out uint LastIlOffset)
    {
        Debug.Assert(symbolReaderHandle != IntPtr.Zero);
        ...
        try
        {
            ...
            var entityHandle =
                MetadataTokens.EntityHandle(MetadataTokens.GetToken(methodDebugInformationHandle.ToDefinitionHandle()));
                ...
                if (reader.GetGuid(cdi.Kind) == asyncMethodSteppingInformationBlob)
                {
                    ...
                    while (blobReader.Offset < blobReader.Length)
                    {
                        list.Add(new AsyncAwaitInfoBlock() {
                            yield_offset = blobReader.ReadUInt32(), resume_offset = blobReader.ReadUInt32(),
            ...
            // We don't use LINQ in order to reduce memory consumption for managed part, so, Reverse() usage not an
            // option here. Note, SequencePointCollection is IEnumerable based collections.
            foreach (SequencePoint p in GetSequencePointCollection(methodToken, reader))
            {
