...
template <typename CharT> class StringViewBase
{
  ...
  private:
    ...
    {
        static size_type length(const CharType *ptr)
        {
            return strlen(ptr);
        }
  ...
  public:
    ...
    constexpr StringViewBase(c_str_wrapper str) : _data(str.str), _end(_data + Traits<value_type>::length(_data))
    {
    }

    // Construct string_view from literal (no need to compute length).
    template <size_type N>
    /*explicit*/ constexpr StringViewBase(const value_type (&literal)[N]) : _data(literal), _end(_data + N - 1)
    {
    }
    ...
    template <typename Traits, typename Allocator> operator std::basic_string<CharT, Traits, Allocator>() const
    {
        return {data(), size()};
    }
    ...
    {
        return *new (this) StringViewBase(other);
    }
    ...
    void swap(StringViewBase &other)
    {
        StringViewBase copy(other);
        other = *this, *this = copy;
    }
    ...
    int compare(StringViewBase s) const
    {
        return Traits<value_type>::compare(*this, s);
    }
    ...
    {
        return compare(StringViewBase(s));
    }
    ...
    {
        return find(StringViewBase(s, count), pos);
    }

    size_type find(const_pointer s, size_type pos = 0) const
    {
        return find(StringViewBase(s), pos);
    }
    ...
    {
        return rfind(StringViewBase(s, count), pos);
    }

    size_type rfind(const_pointer s, size_type pos = npos) const
    {
        return rfind(StringViewBase(s), pos);
    }

    // Function finds the first character equal to any of the characters in the given character sequence.
    // Return value: Position of the first occurrence of any character of the substring, or npos if no such character is
    // found.
    ...
    {
        return find_first_of(StringViewBase(s), pos);
    }
    ...
    //
    // Return value: Position of the last occurrence of any character of the substring, or npos if no such character is
    // found.
    ...
    {
        return find_last_of(StringViewBase(s), pos);
    }
    ...
    template <size_type N> bool starts_with(const value_type (&literal)[N])
    {
        return starts_with({literal, N - 1});
    }

    // for arrays length is computed
    template <size_type N> bool starts_with(value_type (&literal)[N])
    {
    ...
    // for literals length is known
    template <size_type N> bool ends_with(const value_type (&literal)[N])
    {
        return ends_with({literal, N - 1});
    }

    // for arrays length is computed
    template <size_type N> bool ends_with(value_type (&literal)[N])
    {
    ...
    // for literals length is known
    template <size_type N> bool contains(const value_type (&literal)[N])
    {
        return contains({literal, N - 1});
    }

    // for arrays length is computed
    template <size_type N> bool contains(value_type (&literal)[N])
    {
...
template <typename CharT> bool operator==(StringViewBase<CharT> a, StringViewBase<CharT> b)
{
...
template <typename CharT, typename T> bool operator==(StringViewBase<CharT> a, const T &b)
{
    return a.compare(b) == 0;
}
template <typename CharT, typename T> bool operator==(const T &a, StringViewBase<CharT> b)
{
