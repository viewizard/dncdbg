...
template <typename Traits> struct IOSystemImpl
{
    ...
    struct IOResult
    {
        enum Status // NOLINT(performance-enum-size,cppcoreguidelines-use-enum-class)
        {
    ...
    struct AsyncHandleIterator
    {
      public:
        ...
        bool operator==(const AsyncHandleIterator &other) const
        {
            return &**this == &*other;
        }
        bool operator!=(const AsyncHandleIterator &other) const
      ...
      private:
        ...
        template <typename T> struct OpsImpl
        {
        ...
        // can't generate proper code and ASAN detect "ERROR: AddressSanitizer: stack - buffer - underflow on
        // address...".
        union data_align_t {
    ...
    {
        return Traits::listen_socket(tcp_port);
    }

    // Function perform reading from the file: it may read up to `count' bytes to `buf'.
    static IOResult read(FileHandle fh, void *buf, size_t count)
    {
        return Traits::read(fh.handle, buf, count);
    }

    // Function perform writing to the file: it may write up to `count' byte from `buf'.
    static IOResult write(FileHandle fh, const void *buf, size_t count)
    {
        return Traits::write(fh.handle, buf, count);
    }

    // Enable or disable handle inheritance for child processes.
    static IOResult set_inherit(FileHandle fh, bool inherit_handle)
    {
        return Traits::set_inherit(fh.handle, inherit_handle);
    }
    ...
    static AsyncHandle async_read(FileHandle fh, void *buf, size_t count)
    {
        return {Traits::async_read(fh.handle, buf, count)};
    }
    ...
    // for which later result must be obtained via call to `async_result` or
    // operation must be canceled via call to `async_cancel`.
    static AsyncHandle async_write(FileHandle fh, const void *buf, size_t count)
    {
        return {Traits::async_write(fh.handle, buf, count)};
    }
    ...
    static IOResult async_cancel(AsyncHandle &h)
    {
        return {Traits::async_cancel(h.handle)};
    }
    ...
    static IOResult async_result(AsyncHandle &h)
    {
        return {Traits::async_result(h.handle)};
    }

    // Function closes the file represented by file handle.
    static IOResult close(FileHandle fh)
    {
        return Traits::close(fh.handle);
    }

    // This enumeration represents standard IO files.
    enum StdFileType // NOLINT(performance-enum-size,cppcoreguidelines-use-enum-class)
    {
